const WeatherDataLoader = require("../../src/weather/WeatherDataLoader");
...
test('Maximal temperature over hours is correct', async () => {
    ...
    await loader.maxTempWindOverHours({start: startDate, end: endDate}).then(
        res => {
...
test('Max temp and wind speed for a date works', async () => {
    ...
    await loader.maxTempWind(date).then(res => {
